...
```
 ...
 According to this report, line 12 was never run. Line 10 was run 10 times. Now, how is this reporting which lines of code are covered by unit tests? the answer it doesn't, at least not yet. So far you have generated a report on which lines of code are executed after a single run. Next, we are going to use gcov to figure out how well our tests are covering the code. Before we go ahead, commit these changes and push them unto a different branch.
...
$ make
```
This will create two executables: main and test. Now run test by typing `./bin/test`. We need to run the test at least once so we can generate the report.

Now the question is, how do we run Gcov?. The linked files that you need for Gcov is stored inside the CMakeFiles folder. So from inside build, run the following commands:
```
...
-  CMakeLists.txt is not including the Hailstone files right now. Modify line 41 so it looks like this:
```
set(SOURCE_FILES src/testsubject.cpp src/Triangle.cpp src/Hailstone.cpp)
```
Then go to line 44 and change it to this:
```
...
- Add tests so that there are at least three (they should be of different types, not variations of the same assertion) for each function in the cpp files within the src folder (Triangle.cpp, Hailstone.cpp and testsubject.cpp). In the end, the report should show that each line is executed at least once by the tests and each function is executed at least three times.
